...
class NeuralSpline(nn.Module):
	def __init__(self, n, nc, nexperts, colorspace='srgb', apply_to='rgb', abs=False, downsample_strategy='avgpool', dropout=0.0, n_input_channels=3):
		...
		self.xs = torch.arange(0,1,1.0/255.).cpu()
		momentum = 0.01
	...
	def forward(self, batch):
		#print("Hello? - now with info on batch type")
		#print(type(batch))
		#print(batch)
		#print(f"batch has dimension {batch.size()}")
		...
		# out = [torch.zeros(batch.size()).cpu() for i in range(self.nexperts)]
		out = [torch.zeros(batch.size(0),3,batch.size(2),batch.size(3)).cpu() for i in range(self.nexperts)]
		splines = [torch.zeros(batch.size(0),3,self.xs.size(0)) for i in range(self.nexperts)]
		# for each expert
		for nexp in range(self.nexperts):
			# init output vars
			cur_out = torch.zeros(batch.size()).cpu()
